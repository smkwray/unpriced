...
This repo has three related but distinct childcare objects. First, an active-care bridge benchmark scaled to the under-5 population, which keeps the narrower bridge quantity explicit as a benchmark object. Second, a broader annual-hours childcare account, which keeps active household care, active nonhousehold care, and supervisory care explicit and values them in annual hours and dollars. Third, the marketization model, which uses the same active-care bridge quantity class as a solver input rather than as a standalone accounting benchmark. Those are not the same estimand and should not be read as interchangeable.
...
- **Active-care bridge benchmark** asks what a narrower active-care bridge quantity is worth at current replacement prices when scaled to the under-5 population.
...
```
active-care bridge benchmark value = annual replacement price × active-care bridge quantity
...
- The **active-care bridge benchmark** keeps the narrower bridge quantity explicit as a benchmark object.
...
Both preferred benchmarks use a **direct-care-equivalent** price layer rather than the raw gross market price. The gross market price is retained as an upper valuation layer, and a specialist-wage layer is reported alongside it. The direct-care adjustment removes a pooled non-direct residual, but it does **not** separately identify facilities, meals, advertising, transport, administration, profits, or market power. The separate marketization solver keeps using the narrower active-care bridge quantity scaled to the under-5 population; that bridge is not the annual-hours account quantity object, and it is not an under-5-only ATUS time slice.

The annual benchmark series are generated by `python -m unpriced.cli report`. The current output artifact is `outputs/reports/childcare_satellite_account.json`, which contains both benchmark methodologies.
...
- **Unpaid quantity** — the solver currently uses an active-care bridge quantity derived from ATUS active childcare hours and scaled to the under-5 population. The pooled national static account separately reports active household, active nonhousehold, and supervisory childcare hours in annual levels.
